...
class DownConv(nn.Module):
    def __init__(self, seq_len=200, hidden_size=64, m_segments=4,k1=10,channel_reduction=16):
        super().__init__()
        """ 
            DownConv is implemented by stacked strided convolution layers and more details can be found below. When the
            parameters k_1 and k_2 are determined, we can soon get m in Eq.2 of the paper. However, we are more concerned 
            with the size of the parameter m, so we searched for a combination of parameter m and parameter k_1 (parameter 
            k_2 can be easily calculated in this process) to find the optimal segment numbers. 
            Args:
                input_tensor (torch.Tensor): the input of the attention layer

            Returns:
                output_conv (torch.Tensor): the convolutional outputs in Eq.2 of the paper
        """
        self.m =m_segments
        self.k1 = k1
        self.channel_reduction = channel_reduction # avoid over-parameterization
        middle_segment_length = seq_len/k1
        k2=math.ceil(middle_segment_length/m_segments)
        padding = math.ceil((k2*self.m-middle_segment_length)/2.0) # pad the second convolutional layer appropriately
        self.conv1a = nn.Conv1d(in_channels=hidden_size, out_channels=hidden_size // self.channel_reduction,
                                kernel_size=self.k1,
                                stride=self.k1)
        self.relu1a = nn.ReLU(inplace=True)
        self.conv2a = nn.Conv1d(in_channels=hidden_size // self.channel_reduction, out_channels=hidden_size,
                                kernel_size=k2,
                                stride=k2, padding = padding)

    def forward(self, input_tensor):
        input_tensor = input_tensor.permute(0, 2, 1)
        x1a = self.relu1a(self.conv1a(input_tensor))
        x2a = self.conv2a(x1a)
        if x2a.size(2) != self.m:
            print('size_erroe, x2a.size_{} do not equals to m_segments_{}'.format(x2a.size(2),self.m))
        output_conv = x2a.permute(0, 2, 1)
        return output_conv



class ConvMultiHeadAttention(nn.Module):
    def __init__(self, n_heads, m_segments, hidden_size, seq_len, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps,k1):
        ...
        self.attpooling_key = DownConv(seq_len,hidden_size,m_segments,k1)
        self.attpooling_value = DownConv(seq_len,hidden_size,m_segments,k1)

        # initialization for unsymmetrical position encoding
    ...
    def forward(self, input_tensor, pos_emb):
        ...
        # progressive self-attention: to capture global-local sequential patterns efficiently and progressively
        ...
        # unsymmetrical position encoding: relation of positions
        value_layer_pos = self.transpose_for_scores(mixed_value_layer)
        # as shown in the figure1-a (green arrow)
        pos_emb = pos_emb.unsqueeze(0).repeat(input_tensor.size(0),1,1)+input_tensor
...
class paupEncoder(nn.Module):
    ...
    """
    def __init__(self, n_heads, m_segments, hidden_size, seq_len, intermediate_size,
                 hidden_dropout_prob, attn_dropout_prob, hidden_act, layer_norm_eps,k1):
        super(paupEncoder, self).__init__()
        self.multi_head_attention = ConvMultiHeadAttention(n_heads, m_segments, hidden_size,
                                       seq_len, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps,k1)
...
class PAUPEncoder(nn.Module):
    r""" One PAUPEncoder consists of several paupEncoders.
    ...
    def __init__(self,
        ...
        super(PAUPEncoder, self).__init__()
        layer = paupEncoder(n_heads, m_segments, hidden_size, seq_len, inner_size,
                                 hidden_dropout_prob, attn_dropout_prob, hidden_act, layer_norm_eps,k1)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_heads, hidden_size, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps):
        ...
        #print('self.all_head_size', self.all_head_size) # 64
    ...
    def forward(self, input_tensor, attention_mask):
        ...
        query_layer = self.transpose_for_scores(mixed_query_layer)
        key_layer = self.transpose_for_scores(mixed_key_layer) #[1024, 2, 50, 32]
        value_layer = self.transpose_for_scores(mixed_value_layer)

        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))
        #print('attention_scores', attention_scores.size()) #[1024, 2, 50, 50]
